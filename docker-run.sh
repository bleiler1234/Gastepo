...
image_exists=`docker images -a | grep zhonghua.yu/gastepo`
container_exists=`docker ps -a | grep gastepo`
function run_container() {
    if [[ -z "$container_exists" ]];then
        echo "Now create docker container named gastepo..."
        docker create --name gastepo -p 5000:5000 -v /Users/mayer/Software/Docker/Automation/gastepo/config:/Automation/Gastepo/Config -v /Users/mayer/Software/Docker/Automation/gastepo/testcase:/Automation/Gastepo/TestSuite/TestCase zhonghua.yu/gastepo
...
if [[ -z "$image_exists" ]];then
    echo "docker image zhonghua.yu/gastepo is not exist, Now will try to build it..."
    if [[ -z "$dockerfile_exists" ]];then
        echo "[Warning]: Dockerfile is not found on current directory, Please check it first!"
        exit 0
    else
        docker build -f Dockerfile -t zhonghua.yu/gastepo .
        if [[ -n $(docker images -a | grep zhonghua.yu/gastepo) ]];then
            echo "[Success]: docker image zhonghua.yu/gastepo has been built successfully!"
            run_container
        else:
            echo "[Failure]: docker image zhonghua.yu/gastepo build failure, please try again!"
        fi
    fi
else
   run_container
